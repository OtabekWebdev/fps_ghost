...
(function () {
  ...
  const metaHook = esmsInitOptions.meta
    ? globalHook(shimMode && esmsInitOptions.meta)
    : noop;
  ...
  if (Array.isArray(skip)) {
    ...
    skip = (s) =>
      l.some((i) => (i[i.length - 1] === "/" && s.startsWith(i)) || s === i);
  ...
  const throwError = (err) => {
    (self.reportError || (hasWindow && window.safari && console.error) || eoop)(
      err
    ),
      void onerror(err);
  };
                ...
                if (self._esmsi) {
                  ...
                  reject(
                    (!(_err instanceof Event) && _err) ||
                      (err && err.error) ||
                      new Error(
                        `Error loading ${(opts && opts.errUrl) || url} (${
                          s.src
                        }).`
                      )
                  );
  ...
  let supportsImportMaps =
    hasDocument && HTMLScriptElement.supports
  ...
  const featureDetectionPromise = Promise.resolve(dynamicImportCheck).then(
    () => {
      if (
        ...
        (supportsImportMaps && !cssModulesEnabled && !jsonModulesEnabled)
      )
        return;
      ...
      if (!hasDocument)
        return Promise.all([
          ...
          cssModulesEnabled &&
            dynamicImport(
              createBlob(
                cssModulesCheck.replace("x", createBlob("", "text/css"))
              )
  ...
  function parse(l, k = "@") {
                      ...
                      case 41: {
                        ...
                        if (
                          (r << 16) >> 16 != 0
                            ? ((o =
                                f[
                                  ((f[69] | 0) + (((r & 65535) + -1) << 2)) >> 2
                                ] | 0),
                              (f[(o + 20) >> 2] | 0) ==
                                (f[
                                  ((f[68] | 0) + ((a & 65535) << 3) + 4) >> 2
                                ] |
                                  0))
                            : 0
                        ) {
    ...
    for (; e.ri(); ) {
      ...
      let n;
      e.ip() &&
        (n = b(-1 === s ? a : a + 1, t.charCodeAt(-1 === s ? a - 1 : a))),
              ...
              blobUrl = depLoad.s = createBlob(
                `export function u$_(m){${depLoad.a[1]
                  .map(({ s, e }, i) => {
                    ...
                    return `e$_${i}=m${q ? `[` : "."}${depLoad.S.slice(s, e)}${
                      q ? `]` : ""
                    }`;
                  })
                  .join(",")}}${
